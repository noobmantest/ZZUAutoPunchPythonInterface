...
print(time.asctime(time.localtime(time.time())), "--准备加载平台主页--", id)
r = requests.post("https://jksb.v.zzu.edu.cn/vls6sss/zzujksb.dll/jksb", headers=headers, data=form)  # response为打卡的第二个表单
print(time.asctime(time.localtime(time.time())), "--成功加载平台主页--", id)
...
print(time.asctime(time.localtime(time.time())), "--准备提交打卡表单--", id)
r = requests.post("https://jksb.v.zzu.edu.cn/vls6sss/zzujksb.dll/jksb", data=form, headers=headers)  # response为完成打卡页面
print(time.asctime(time.localtime(time.time())), "--成功提交提交打卡表单--", id)

text = r.text.encode(r.encoding).decode(r.apparent_encoding)  # 解决乱码问题
# print(text)
if "感谢你今日上报健康状况！" in text:
    print(time.asctime(time.localtime(time.time())), "--打卡成功--", id)
elif '“地市”与“省份”选择相矛盾' in text:
    print(time.asctime(time.localtime(time.time())), "--打卡失败--地市与省份选择相矛盾--", id)
else:
    print(time.asctime(time.localtime(time.time())), "--打卡失败--", id)
    print(time.asctime(time.localtime(time.time())), "--打卡失败返回的页面如下--", id)
    print(text)
exit()
